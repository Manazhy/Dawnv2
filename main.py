...
class Dawn:
    ...
    def welcome(self):
        print(
            f"""
        {Fore.GREEN + Style.BRIGHT}Dawn - BOT (SINAN) |  {Fore.YELLOW + Style.BRIGHT}Tool BY XYZ CRIMINAL  (CRYPTOCRACKERS )
            """
        )
    ...
    async def question(self):
        while True:
            try:
                print("Input number => Enter |  => Enter")
                print("1. Run With Auto Proxy | Proxy share (Free)")
                print("2. Run With Manual Proxy | optional")
                print("3. Run  Proxy | Private")
